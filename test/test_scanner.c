...
static void probar_token(const char *input, int expected_token) {
	...
	TEST_ASSERT_EQUAL_INT_MESSAGE(expected_token, actual_token,
	                              "El token devuelto no es el esperado");
	TEST_ASSERT_EQUAL_STRING_MESSAGE(input, yytext,
	                                 "El texto consumido (yytext) no coincide");
